...
int TextRecognizer::recognize(IplImage *input,
	...
	for (unsigned int i = 0; i < chainBB.size(); i++) {
		cv::Point center = cv::Point( (chainBB[i].first.x + chainBB[i].second.x /2),
		...
		{
			LOGL(LOG_TXT_ORIENT, (chainBB[i].second.x - chainBB[i].first.x) << " < " << (input->width / params.maxImgWidthToTextRatio));
			continue;
		}
		...
		cv::Mat inputMat = cv::Mat(grayImage);
		...
		cv::Rect roi = getBoundingBox(compCoords, cv::Size(input->width, input->height) );
